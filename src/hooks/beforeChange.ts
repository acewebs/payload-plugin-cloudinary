...
export const beforeChange: BeforeChangeHook = async ({
  data,
  req,
  originalDoc,
  collection,
}) => {
  logCloudinary("[CloudinaryPlugin] beforeChange image called ", {
    data,
    req,
    originalDoc,
  });
  ...
  if (cloudinaryPublicId) {
    try {
      ...
      req.payload.logger.warn(
        "[CloudinaryPlugin] Failed to delete old asset:",
        err
      );
    }
  }
  ...
  if (!buffer) {
    req.payload.logger.error(
      "[CloudinaryPlugin] No file buffer found for upload"
    );
    return data;
  }
  ...
  try {
    ...
    logCloudinary("[CloudinaryPlugin] Upload successful, with result: ", {
      ...uploadResult,
    });
